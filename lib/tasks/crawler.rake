namespace :crawler do
  ...
  task save_news: :environment do
    ...
    posts_hash = posts.map {|post| {'title' => post['text'], 'description' => post['short_content']['short_desc'], 'original_url' => post['link'], 'post_cover' => post['short_content']['thumbnail']}}
    posts_hash.each do |post|
      Post.create(post.merge(content: post['description'], category: 'technology', user_id: User.first.id, post_cover: post['post_cover']))
    end
